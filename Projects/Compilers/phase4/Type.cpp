...
{
	return (_specifier == INT || _specifier == DOUBLE) && _indirection == 0 && _kind == SCALAR;
	//return (promote().specifier() == INT || _specifier == DOUBLE) && _kind == SCALAR;	
}
...
{
	return (isNumeric() && that.isNumeric()) || (isPredicate() && *this == that);
}
