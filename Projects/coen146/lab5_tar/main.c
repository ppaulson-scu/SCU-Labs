	...
	while(1){
		...
		for(i = 0; i < N; i++){
			...
			for(j = 0; j < N; j++){
				...
				sendMessage(i, neighbor_table[i][j]);
			}	
		}
	}
}
...
void sendMessage(int id, int cost){
	UDPSocket mysocket;
	struct addrinfo hints, *res;
	char str[100];
	message msg;
	...
	sendto(mysocket.sockfd, str, sizeof(str) - 1, 0, res->ai_addr, res->ai_addrlen);
	fprintf(stderr, "Transmitted to router %d: %d %d %d\n", id, msg.src, msg.dest, msg.cost);
